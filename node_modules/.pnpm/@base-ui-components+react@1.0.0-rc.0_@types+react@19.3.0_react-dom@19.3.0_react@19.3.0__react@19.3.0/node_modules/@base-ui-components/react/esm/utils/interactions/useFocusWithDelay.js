 ...
 */
export function useFocusWithDelay(store, props = {}) {
  ...
  const reference = React.useMemo(() => ({
    ...
    onBlur(event) {
      ...
      timeout.start(0, () => {
        const currentDomReference = store.select('domReferenceElement');
        const activeEl = activeElement(currentDomReference ? currentDomReference.ownerDocument : document);
        ...
        if (contains(dataRef.current.floatingContext?.refs.floating.current, activeEl) || contains(currentDomReference, activeEl)) {
          return;
        }
