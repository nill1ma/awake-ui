import fs, { promises, existsSync, readFileSync, createReadStream } from 'node:fs';
...
import { slash as slash$1, deepMerge, cleanUrl as cleanUrl$1, toArray } from '@vitest/utils/helpers';
import sirv from 'sirv';
import c from 'tinyrainbow';
import { isFileLoadingAllowed, createDebugger, isValidApiRequest, BrowserConnectionError, isCSSRequest, distDir, rolldownVersion, isFileServingAllowed } from 'vitest/node';
...
function BrowserContext(contribution) {
	return {
		...
		load(id) {
			const globalServer = contribution.parent;
...
async function generateContextFile(globalServer) {
	...
	if (!providerName) {
		throw new Error(`Browser provider is not defined for the project "${child.project.name}". This is a bug in Vitest. Please, open a new issue with a reproduction.`);
	}
...
async function getUserEventImport(provider, resolve) {
	...
	return `\
import { userEvent as _userEventSetup } from '${slash$1(`/@fs/${resolved.id}`)}'
`;
}

var BrowserPlugin = (contribution) => {
	return [
		{
		...
		},
		{
			...
			transform(code, id) {
				const parentServer = contribution.parent;
	...
	constructor(ctx, base) {
		...
		this.stackTraceOptions = {
			...
			getSourceMap: (id) => {
				...
				if (result && (!result.map || result.map.mappings === "" || filePath.startsWith(this.vite.config.cacheDir))) {
		...
		this.orchestratorHtml = (this.config.browser.ui ? readFile$1(resolve(distClientRoot, "index.html"), "utf8") : readFile$1(resolve(distRoot, "client/orchestrator.html"), "utf8")).then((html) => this.orchestratorHtml = html);
...
const DEFAULT_HEARTBEAT_INTERVAL = 15e3;
const HEARTBEAT_MAX_MISSED = 2;
let warnedInvalidHeartbeatInterval = false;
function resolveHeartbeatInterval(vitest) {
...
function setupBrowserRpc(globalServer, defaultMockerRegistry) {
		...
		wss.handleUpgrade(request, socket, head, (ws) => {
			...
			const heartbeat = heartbeatInterval > 0 ? setInterval(() => {
				...
				if (missedPongs >= HEARTBEAT_MAX_MISSED) {
					...
					rpc.$close(new Error(`[vitest] The browser ${type} did not respond to a heartbeat ping for ${missedPongs * heartbeatInterval}ms. The browser process might be frozen or killed. Closing the connection.`));
					ws.terminate();
					return;
				}
				missedPongs++;
				ws.ping();
			}, heartbeatInterval).unref() : undefined;
			ws.on("close", () => {
				...
				rpc.$close(new BrowserConnectionError(`[vitest] Browser connection was closed while running tests. Was the page closed unexpectedly?`));
			});
		});
	});
	...
	function assertCdpAllowed(project) {
		if (!isCdpAllowed(project)) {
			throw new Error(`Cannot use CDP because browser API write or exec operations are disabled. See https://vitest.dev/config/api.`);
		}
	}
	function setupClient(project, rpcId, ws, options) {
		...
		const rpc = createBirpc({
			...
			async onTaskArtifactRecord(id, artifact) {
				if (!canWrite(project)) {
					if (artifact.type === "internal:annotation" && artifact.annotation.attachment) {
						...
						vitest.logger.error(`[vitest] Cannot record annotation attachment because file writing is disabled. See https://vitest.dev/config/api.`);
					}
					...
					if (artifact.attachments?.length) {
						...
						vitest.logger.error(`[vitest] Cannot record attachments ("${attachments}") because file writing is disabled, removing attachments from artifact "${artifact.type}". See https://vitest.dev/config/api.`);
					}
				} else {
			...
			async writeBenchmarkResult(relativePath, data) {
				if (!canWrite(project)) {
					vitest.logger.error(`[vitest] Cannot write benchmark artifact "${relativePath}" because file writing is disabled. See https://vitest.dev/config/api.`);
					return;
				}
				checkFileAccess(project.benchmark.resolve(relativePath));
			...
			async readSnapshotFileData(snapshotPath) {
				...
				if (!canExec(project)) {
					throw new Error(`Cannot read snapshot file because browser API exec operations are disabled. See https://vitest.dev/config/api.`);
				}
			...
			async saveSnapshotFile(id, content) {
				checkFileAccess(id);
				if (!canWrite(project)) {
					vitest.logger.error(`[vitest] Cannot save snapshot file "${id}". File writing is disabled because server is exposed to the internet, see https://vitest.dev/config/api.`);
					return;
				}
			...
			async removeSnapshotFile(id) {
				checkFileAccess(id);
				if (!canWrite(project)) {
					vitest.logger.error(`[vitest] Cannot remove snapshot file "${id}". File writing is disabled because server is exposed to the internet, see https://vitest.dev/config/api.`);
					return;
				}
				if (!existsSync(id)) {
			...
			async triggerCommand(sessionId, command, testPath, payload) {
				...
				const context = Object.assign({
					...
					contextId: sessionId,
					sessionId,
					mark: async (name, options) => {
						const tester = project.browser.state.testers.get(rpcId);
...
function pinCacheControl(res, value) {
  ...
  min-height: 100vh;
		...
		configureServer(server) {
			...
			server.middlewares.use((req, _res, next) => {
				if (req.url?.includes("_vitest_original") && parentServer.config.browser.provider?.name === "playwright") {
			...
			const persistentContext = (parentServer.config.browser.provider?.options)?.persistentContext;
		...
		createParent({ config, vitest }) {
			if (vitest.version !== version) {
				vitest.logger.warn(c.yellow(`Loaded ${c.inverse(c.yellow(` vitest@${vitest.version} `))} and ${c.inverse(c.yellow(` @vitest/browser@${version} `))}.` + "\nRunning mixed versions is not supported and may lead into bugs" + "\nUpdate your dependencies and make sure the versions match."));
			}
			return new ParentBrowserProject({
				config,
				vitest
			}, "/");
		},
		setupRpc(parent) {
			setupBrowserRpc(parent, mockerRegistry);
		}
	};
	contribution.plugins = [
		...
		}),
		{
			...
			transform(code, id) {
				...
				if (!parentServer || !isHeadlessServer(parentServer) || parentServer.vitest.config.inspector.enabled) {
					return null;
				}
				if (isCSSRequest(id)) {
					return null;
				}
				const path = cleanUrl$1(id);
				if (path.startsWith(distRoot) || path.startsWith(distDir)) {
					return {
						code,
						map: { mappings: "" }
					};
				...
				if (parentServer.config.browser.dependencySourcemaps === false && path.includes("/node_modules/") && (parentServer.vite.config.server.sourcemapIgnoreList(path, path) ?? true)) {
					return {
						code,
						map: { mappings: "" }
					};
...
function isHeadlessServer(parentServer) {
...
function resolveBrowserOptimizeDeps(projectConfigs, testFiles, harness) {
	...
	return {
		...
		exclude,
		include,
		...rolldownVersion ? { rolldownOptions: { plugins: [rolldownPlugin] } } : { esbuildOptions: { plugins: [esbuildPlugin] } }
	};
}
...
export { assertBrowserApiWrite, assertBrowserFileAccess, createBrowserServer, defineBrowserCommand, defineBrowserProvider, parseKeyDef, resolveScreenshotPath };
