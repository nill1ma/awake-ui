//#region ../../node_modules/.pnpm/rrweb-snapshot@2.1.1_patch_hash=b66b30796877352a5f887f3f4115c4e7265ddc11772af2223da4bd92293716f6/node_modules/rrweb-snapshot/dist/rrweb-snapshot.js
...
function mutationObserverCtor() {
	return [getUntaintedPrototype("MutationObserver").constructor, untaintedBaseIframeCleanup["MutationObserver"] ?? (() => {})];
}
...
function getValidTagName(element) {
	if (Object.prototype.toString.call(element) === "[object HTMLFormElement]") return "form";
	const processedTagName = toLowerCase(element.tagName);
	if (tagNameRegex.test(processedTagName)) return "div";
	return processedTagName;
}
var canvasServices = /* @__PURE__ */ new WeakMap();
...
function adaptCssForReplay(cssText, cache) {
	...
	let result2 = cssText;
	try {
		result2 = postcss([mediaSelectorPlugin, pseudoClassPlugin]).process(cssText).css;
	} catch (error) {
...
function isSupportedSandboxedIframe(frameElement) {
	...
	if (!("sandbox" in frameElement)) return false;
...
function assertRebuildTargetAllowed(options) {
	if (options.UNSAFE_allowUnprotectedRebuild) return;
	const win = options.doc.defaultView;
	if (!win) return;
	if (sandboxedRebuildDocuments.has(options.doc) && isSupportedSandboxedIframe(win.frameElement)) return;
...
function createSandboxedIframe(options) {
	var _a;
	if (!options.root.isConnected) throw new Error(SANDBOXED_IFRAME_ROOT_ERROR);
	...
	if (!doc || !iframe.contentWindow) {
		(_a = iframe.parentNode) == null || _a.removeChild(iframe);
...
//#endregion
export { IGNORED_NODE, Mirror, absolutifyURLs, adaptCssForReplay, buildNodeWithSN, classMatchesRegex, cleanupSnapshot, createCache, createMirror, createSandboxedIframe, escapeImportStatement, extractFileExtension, fixSafariColons, genId, getInputType, ignoreAttribute, is2DCanvasBlank, isCSSImportRule, isCSSStyleRule, isElement, isNativeShadowDom, isNodeMetaEqual, isShadowRoot, markCssSplits, maskInputValue, needMaskingText, normalizeCssString, rebuild, rebuildIntoSandboxedIframe, serializeNodeWithId, slimDOMDefaults, snapshot, splitCssText, stringifyRule, stringifyStylesheet, toLowerCase, transformAttribute, visitSnapshot };