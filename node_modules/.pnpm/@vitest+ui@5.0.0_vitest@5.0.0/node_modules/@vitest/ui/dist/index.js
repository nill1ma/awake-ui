...
import { d as distClientRoot } from './paths-DmDZ0-yr.js';
import 'node:url';
...
var index = (harness) => {
	if (harness.version !== version) {
		harness.logger.warn(c.yellow(`Loaded ${c.inverse(c.yellow(` vitest@${harness.version} `))} and ${c.inverse(c.yellow(` @vitest/ui@${version} `))}.` + "\nRunning mixed versions is not supported and may lead into bugs" + "\nUpdate your dependencies and make sure the versions match."));
	}
	return {
		name: "vitest:ui",
		apply: "serve",
		configureServer: {
			...
			handler(server) {
				...
				function serializeTokenCookie() {
					return distExports.serialize(UI_TOKEN_COOKIE, ctx.config.api.token, {
						path: base,
						httpOnly: true,
						sameSite: "strict"
					});
				}
				function hasValidTokenCookie(req) {
					const cookieToken = distExports.parse(req.headers.cookie ?? "")[UI_TOKEN_COOKIE];
					if (!cookieToken) {
						return false;
					}
					try {
						return crypto.timingSafeEqual(Buffer.from(cookieToken), Buffer.from(ctx.config.api.token));
					} catch {
						return false;
					}
				}
				...
				server.middlewares.use(base, sirv(distClientRoot, {
					single: true,
					dev: true
				}));
			}
...
export { index as default, distClientRoot };
